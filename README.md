...
## Welcome!
Welcome to the official repository for the Archipelago integration manual for **Super Street Fighter II**.  
This community-driven project enhances the gaming experience by linking objectives and items across multiple games in the Archipelago network.  
Currently, the randomizer supports the SNES version of Super Street Fighter II. Other versions will be reviewed for compatibility (some modes, like *Time Challenge*, may not exist in every version).  
Hook support for more advanced YAML customization is planned.

## Project Status
We’ve moved from **Alpha** to **Beta**!  
The core functionality is complete and stable, with ongoing optimizations and new features under development.  
Every Beta release is tested internally before publishing, but bugs can still slip through — please report any issues you encounter.
...
### Version 0.8.0 – *New Logique* Update
- Made some categories visible again in the client via `categories.json`.
- Logic overhaul:  
  - Certain challenges (e.g., getting stunned, or defeating a specific character with another character) are now possible in *Time Challenge* mode.  
  - The logic now also requires the CPU version of the character when in this mode.
- Changed the syntax of all `requires` entries.
- Added a **Stage** check category and its related checks.
- Added **First Attack** checks for each character.
- Added subcategories for special moves in `item.json`.
- Added checks related to special moves.
- Special moves are now progression items.
- In the client, all special moves will appear in the **Special Moves** category for clarity (no more one category per character).
...
## Contact
Questions or streams? Reach out on the Archipelago Discord or open an issue here on GitHub. I’ll respond as soon as possible.  
